...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "Vault"; //Local Storage Names for PWA
...
submenus();
		...
		var headerDateElement = document.querySelector('.header-date');

		if (greetingElement && headerDateElement) {
			...
			// Get day, month, and suffix for the date
			var dayName = currentDate.getDate();
			var daySuffix = 'th';
			if (dayName === 1 || dayName === 21 || dayName === 31) { daySuffix = 'st'; }
			if (dayName === 2 || dayName === 22) { daySuffix = 'nd'; }
			if (dayName === 3 || dayName === 23) { daySuffix = 'rd'; }
			...
			// Set the greeting and date separately in the HTML elements
			greetingElement.textContent = greeting;  // Use textContent to update the greeting
			headerDateElement.textContent = weekdayName[currentDate.getDay()] + ' ' + dayName + daySuffix + ' ' + monthNames[currentDate.getMonth()];
		}

		// Accessing the greeting later
		function getGreeting() {
			var greetingElement = document.querySelector('.greeting');
			if (greetingElement) {
				return greetingElement.innerHTML; // Returns the greeting content
			}
